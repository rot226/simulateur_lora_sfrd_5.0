    ...
    def start_reception(
        ...
        *,
        ...
        :param orthogonal_sf: Si ``True``, les transmissions de SF différents
            sont ignorées pour la détection de collision. Si ``False``, les
            transmissions actives sur la même fréquence sont toutes prises en
            compte, quel que soit leur SF.
        """
        key = (sf, frequency)
        if orthogonal_sf:
            ...
            concurrent_transmissions = []
            for (sf_k, freq_k), tx_list in self.active_map.items():
                if freq_k != frequency:
                    continue
                for t in tx_list:
                    if t["end_time"] > current_time:
                        concurrent_transmissions.append(t)
        ...
        if capture_mode in {"advanced", "omnet"} and noise_floor is not None:
            ...
            winners = flora_phy.capture(
                rssi_list, sf_list, start_list, end_list, freq_list
            )
            capture = any(winners)
            if capture:
        ...
        if capture:
            ...
            for t in interfering_transmissions:
                if t["lost_flag"]:
                    k = (t.get("sf", sf), t.get("frequency", frequency))
                    try:
                        self.active_map[k].remove(t)
